...
from  model_cat_layers import embed_net
from utils import *
import time 
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/sysu_cat_layers_nolr/', type=str, help='model save path')
...
parser.add_argument('--gall-mode', default='multi', type=str, help='single or multi')
...
start_epoch = 0 
print('==> sysu_cat_layers_nolr_all_multi..')
...
def extract_gall_feat(gall_loader):
    ...
    gall_x_cat = np.zeros((ngall, 3*pool_dim))
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            pool_feat, feat,x_cat = net(input, input, test_mode[0])
            gall_feat[ptr:ptr+batch_num,: ] = feat.detach().cpu().numpy()
            gall_feat_pool[ptr:ptr+batch_num,: ] = pool_feat.detach().cpu().numpy()
            gall_x_cat[ptr:ptr+batch_num,: ] = x_cat.detach().cpu().numpy()
            ptr = ptr + batch_num
    print('Extracting Time:\t {:.3f}'.format(time.time() - start), file=test_log_file)
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
    return gall_feat, gall_feat_pool,gall_x_cat
...
def extract_query_feat(query_loader):
    ...
    query_x_cat = np.zeros((nquery, 3*pool_dim))
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            pool_feat, feat,x_cat = net(input, input, test_mode[1])
            query_feat[ptr:ptr+batch_num,: ] = feat.detach().cpu().numpy()
            query_feat_pool[ptr:ptr+batch_num,: ] = pool_feat.detach().cpu().numpy()
            query_x_cat[ptr:ptr+batch_num,: ] = x_cat.detach().cpu().numpy()
            ptr = ptr + batch_num         
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
    print('Extracting Time:\t {:.3f}'.format(time.time() - start), file=test_log_file)
    return query_feat, query_feat_pool ,query_x_cat
    
query_feat, query_feat_pool ,query_x_cat = extract_query_feat(query_loader)    
...
if dataset =='regdb':
  for trial in range(1, 11):
    ...
    for trial in range(10):
        ...
        gall_feat, gall_feat_pool,gall_x_cat = extract_gall_feat(trial_gall_loader)
        ...
        # pool5 feature
        distmat_pool = np.matmul(query_feat_pool, np.transpose(gall_feat_pool))
        distmat_x_cat = np.matmul(query_x_cat, np.transpose(gall_x_cat))
        cmc_pool, mAP_pool = eval_sysu(-distmat_pool, query_label, gall_label,query_cam, gall_cam)
        cmc_x_cat, mAP_x_cat = eval_sysu(-distmat_x_cat, query_label, gall_label,query_cam, gall_cam)
        
        if trial ==0:
            all_cmc = cmc
            all_mAP = mAP
            all_cmc_pool = cmc_pool + cmc_x_cat
            all_mAP_pool = mAP_pool + mAP_x_cat
        else:
            all_cmc = all_cmc + cmc
            all_mAP = all_mAP + mAP
            all_cmc_pool = all_cmc_pool + cmc_pool + cmc_x_cat
            all_mAP_pool = all_mAP_pool + mAP_pool + mAP_x_cat
