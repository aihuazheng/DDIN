...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50'):
        ...
        if arch =='resnet18':
            ...
            self.visible_net = visible_net_resnet(arch = arch)
            self.thermal_net = thermal_net_resnet(arch = arch)
            self.visible_layer4 = visible_layer4(arch = arch)
            self.thermal_layer4 = thermal_layer4(arch = arch)
            pool_dim = 2048
    ...
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            ...
            x1_s3 = ConvDY2d(x1_s3 ,x2_s3)
            x2_s3 = ConvDY2d(x2_s3 ,x1_s3)
            x1 = self.visible_layer4(x1_s3)
            x2 = self.thermal_layer4(x2_s3)
