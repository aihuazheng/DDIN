...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50',lma = True):
        ...
        self.feature = FeatureBlock(2*pool_dim, low_dim, dropout=drop)
    ...
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            ...
            if self.lma:
              #pdb.set_trace()
              layer_attr_123 = self.wpa(x_r_123, r_s3, 1, 3)
              layer_atti_123 = self.wpa(x_i_123, i_s3, 1, 3)
              
            layer_att_123 = torch.cat((layer_attr_123, layer_atti_123), 0)
            x_s4 = torch.cat((r_s4, i_s4), 0)
            
            layer_att = torch.cat((layer_att_123,x_s4), 1)
            ...
            s_cat_123= torch.cat((x_s1,x_s2,x_s3),0)
        
            
            if self.lma:
              layer_att_123 = self.wpa(s_cat_123, s3, 1, 3)
            ...
            layer_att = torch.cat((layer_att_123,x_s4), 1)  #,64 2048*2
        ...
        if self.training:
            ...
            layer_att =  self.l2norm(layer_att)#64 2048*2
