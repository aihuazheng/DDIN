...
from model_s3_s4_HU_allpool_lr import embed_net
from utils import *
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/s3_s4_HU_allpool_lr/', type=str, 
...
ignored_params = list(map(id, net.feature1.parameters())) \
                 ...
                 + list(map(id, net.fc_output2.parameters())) \
                 ...
                 + list(map(id, net.classifiers4.parameters()))
...
if args.optim == 'sgd':
    optimizer = optim.SGD([
         ...
         {'params': net.fc_output2.parameters(), 'lr': args.lr},
         ...
         {'params': net.classifiers4.parameters(), 'lr': args.lr}],
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        torch.autograd.backward([loss0, loss1, loss2, loss3, loss4, loss5,loss6,loss7], [torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda()])
        #loss.backward()
        optimizer.step()
        loss = (loss0 + loss1 + loss2 + loss3 + loss4 + loss5 +loss6+loss7) / 8
...
def test(epoch):   
    ...
    start = time.time()
    ptr = 0
    gall_feat = np.zeros((ngall, 8*args.low_dim))
    with torch.no_grad():
    ...
    start = time.time()
    ptr = 0
    query_feat = np.zeros((nquery, 8*args.low_dim))
    with torch.no_grad():
