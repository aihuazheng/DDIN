...
from model_s1s2s3s4_multi_class import embed_net
from utils import *
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/model_s1s2s3s4_multi_class_nolr/', type=str, 
...
print('==> sysu_s1s2s3s4_multi_class_nolr..')
print('==> Loading data..')
# Data loading code
...
ignored_params = list(map(id, net.feature1.parameters())) \
                 ...
                 + list(map(id, net.features4.parameters())) \
...
if args.optim == 'sgd':
    optimizer = optim.SGD([
         ...
         {'params': net.features4.parameters(), 'lr': args.lr},
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        torch.autograd.backward([loss0, loss1, loss2, loss3, loss4, loss5,loss6,loss7,loss8,loss9], [torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda()])
        #loss.backward()
        optimizer.step()
...
def test(epoch):   
    ...
    start = time.time()
    ptr = 0
    gall_feat = np.zeros((ngall, 10*args.low_dim))
    with torch.no_grad():
    ...
    start = time.time()
    ptr = 0
    query_feat = np.zeros((nquery, 10*args.low_dim))
    with torch.no_grad():
