...
from model_LWPA_0618 import embed_net
from utils import *
...
parser.add_argument('--model_path', default='/home/panpeng/fmy/save_model/base_ye/LWPA_0618_uploss_nolr.py/', type=str, 
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        if args.method =='id':
            ...
            loss_lwpa = criterion(outputs[6], labels)
            ...
            het_feat_lwpa = feat[6].chunk(2, 0)
            ...
            loss_c_lwpa = criterion_het(het_feat_lwpa[0], het_feat_lwpa[1], label1, label2)
            ...
            loss_lwpa = loss_lwpa + w_hc * loss_c_lwpa
        ...
        optimizer.zero_grad()
        torch.autograd.backward([loss0, loss1, loss2, loss3, loss4, loss5,loss_lwpa], [torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda()])
        #loss.backward()
        optimizer.step()
        loss = (loss0 + loss1 + loss2 + loss3 + loss4 + loss5 + loss_lwpa) / 7
