...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50'):
        ...
        self.featurecat = FeatureBlock(4*pool_dim, low_dim, dropout=drop)
        ...
        self.classifiercat = ClassBlock(low_dim, class_num, dropout=drop)
    ...
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            ...
            x_cat = torch.cat((x_s1, x_s2,x_s3,x_s4), 1)
        ...
        out_cat = self.classifiercat(y_cat)

     
        #out = self.classifier(y)
        if self.training:
            return (out_0, out_1, out_2, out_3, out_4, out_5,out_cat), (self.l2norm(y_0), self.l2norm(y_1), self.l2norm(y_2), self.l2norm(y_3), self.l2norm(y_4), self.l2norm(y_5),self.l2norm(y_cat))
        else:
            x_0 = self.l2norm(x_0)
            x_1 = self.l2norm(x_1)
            x_2 = self.l2norm(x_2)
            ...
            y_4 = self.l2norm(y_4)
            y_5 = self.l2norm(y_5)
            y_cat = self.l2norm(y_cat)
          
            y = torch.cat((y_0, y_1, y_2, y_3, y_4, y_5,y_cat), 1)
            return x, y,x_cat
