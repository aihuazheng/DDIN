...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/model_s1s2s3s4_multi_class_nolr/', type=str, help='model save path')
...
start_epoch = 0 
print('==> sysu_s1s2s3s4_multi_class_nolr_all_multi..')
...
def extract_gall_feat(gall_loader):
    ...
    start = time.time()
    ptr = 0
    gall_feat = np.zeros((ngall, 10*feature_dim))
    gall_feat_pool = np.zeros((ngall, 10*pool_dim))
    with torch.no_grad():
...
def extract_query_feat(query_loader):
    ...
    start = time.time()
    ptr = 0
    query_feat = np.zeros((nquery, 10*feature_dim))
    query_feat_pool = np.zeros((nquery, 10*pool_dim))
    with torch.no_grad():
