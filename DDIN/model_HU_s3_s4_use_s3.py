...
class visible_net_resnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_s3 = x
        x_s3_avg = self.visible.avgpool(x)
...
class thermal_net_resnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_s3 = x
        x_s3_avg= self.thermal.avgpool(x)
        x_s3_f = x_s3_avg.view(x_s3_avg.size(0), x_s3_avg.size(1))
        #x = self.thermal.layer4(x)
        #x_s4 = x
        return x_s3,x_s3_f
        
class share_layer4(nn.Module):
    def __init__(self, arch ='resnet18'):
        super(share_layer4, self).__init__()
        if arch =='resnet18':
            model_ft = models.resnet18(pretrained=True)
        elif arch =='resnet50':
            model_ft = models.resnet50(pretrained=True)
        ...
        for i in range(self.num_orders):
            name = "HAM_" + str(i)
            setattr(self, name, change_HighOrderAttention(2048, i+1))
        '''
    def forward(self, x):
        
        
        x = self.common_net.layer4(x)
        '''
        if self.training:
            bs = x.size()[0] // 2
            rgb = x[:bs]
            ir = x[bs:]
            rgb_ir = self.CMA(rgb, ir)
            ir_rgb = self.CMA(ir, rgb)
            x = torch.cat((rgb_ir, ir_rgb), 0)
        #pdb.set_trace()
        ...
        for i in range(self.num_orders):
            ...
            mid_feat.append(layer(x))
        x = torch.cat((mid_feat), 0)
        '''
    
       # x = self.common_net.avgpool(x)
...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50'):
        ...
        if arch =='resnet18':
            ...
            self.visible_net = visible_net_resnet(arch = arch)
            self.thermal_net = thermal_net_resnet(arch = arch)
            self.layer4 = share_layer4(arch=arch)
            pool_dim = 2048
            pool_s3 =1024
        ...
        #self.fc_output = FeatureBlock(pool_s3 ,low_dim, dropout=drop)
    ...
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            #pdb.set_trace()
            x1_s3,x1_s3_f = self.visible_net(x1)
            x2_s3,x2_s3_f = self.thermal_net(x2)
            
            x1_s3_f_new= self.id_fc1(x1_s3_f)
            x1_s3_f_new = self.id_tanh(x1_s3_f_new)
            x_s3_r = self.conv_basis(x1_s3, x1_s3_f_new)
            
            x1_s3 = self.Conv1x1_r(x1_s3)
            x_s3_r = x_s3_r+x1_s3
            x_s3_r = self.relu(x_s3_r)
            x_s3_r = self.conv1(x_s3_r)
            ...
            x_s3_i = self.conv_basis(x2_s3, x2_s3_f_new)
            x2_s3 = self.Conv1x1_i(x2_s3)
            x_s3_i = x_s3_i+x2_s3
            x_s3_i = self.relu(x_s3_i)
            x_s3_i = self.conv2(x_s3_i)
            #x_s3_i = self.avgpool(x_s3_i)
            #x_s3_i = x_s3_i.view(x_s3_i.size(0), -1)
            #x_s3_i_f = x_s3_i
            
            #x_s3_f = torch.cat((x_s3_r_f,x_s3_i_f),0)  #64*512
            #pdb.set_trace()
            x1_s4 = self.layer4(x_s3_r)
            x2_s4 = self.layer4(x_s3_i)
        ...
        # x = self.dropout(x)
            ...
            #pdb.set_trace()
            x_s3,x_s3_f = self.visible_net(x1)
            ...
            x_s3,x_s3_f= self.thermal_net(x2)
            x_s4 = self.layer4(x_s3)
            x = self.num_part(x_s4)
            x = x.chunk(6, 2)
            x_0 = x[0].contiguous().view(x[0].size(0), -1)
