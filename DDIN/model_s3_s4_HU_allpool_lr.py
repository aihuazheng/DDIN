...
class thermal_net_resnet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.gm_pool  == 'on':
            ...
            x_s3_f = self.thermal.avgpool(x_s3)
            x_s3_f = x_s3_f.view(x_s3_f.size(0), x_s3_f.size(1))
...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50',gm_pool = 'on'):
        ...
        self.id_fc1 = nn.Linear(1024, 64)
        self.id_fc2 = nn.Linear(2048, 64)
        self.id_tanh = nn.Tanh()
        self.conv_basis3= ConvBasis2d(64,1024, 512, kernel_size=1, padding=0, bias=False)
        self.conv_basis4= ConvBasis2d(64,2048, 512, kernel_size=1, padding=0, bias=False)
        #self.conv_basis_i= ConvBasis2d(1024,1024, 512, kernel_size=1, padding=0, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.fc_output1 = nn.Linear(1024,512)
        self.fc_output2 = nn.Linear(2048,512)
        self.num_part = num_part(6)
        self.avgpool= nn.AdaptiveAvgPool2d((1,1))
        self.avgpool2 = nn.AdaptiveAvgPool2d((1,1))
        self.gm_pool = gm_pool
        self.Conv1x1_3 = Conv1x1(1024,512)
        self.Conv1x1_4 = Conv1x1(2048,512)
    ...
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            ...
            #s3 ir to rgb
            x2_s3_f_new = self.id_fc1(x2_s3_f)
            x2_s3_f_new = self.id_tanh(x2_s3_f_new)
            x_s3_r = self.conv_basis3(x1_s3, x2_s3_f_new)
            x1_s3_new = self.Conv1x1_3(x1_s3)
            x_s3_r = x_s3_r+x1_s3_new
            x_s3_r = self.relu(x_s3_r)
            if self.gm_pool  == 'on':
                b, c, h, w = x_s3_r.shape
                x_s3_r = x_s3_r.view(b, c, -1)
                p = 3.0
                x_s3_r = (torch.mean(x_s3_r**p, dim=-1) + 1e-12)**(1/p)
            else:
                x_s3_r = self.avgpool(x_s3_r)
                x_s3_r = x_s3_r.view(x_s3_r.size(0), x_s3_r.size(1))
            x_s3_r_f = x_s3_r
            
            # s3 rgb to ir
            x1_s3_f_new = self.id_fc1(x1_s3_f)
            x1_s3_f_new = self.id_tanh(x1_s3_f_new)
            x_s3_i = self.conv_basis3(x2_s3, x1_s3_f_new)
            x2_s3_new = self.Conv1x1_3(x2_s3)
            x_s3_i = x_s3_i+x2_s3_new
            x_s3_i = self.relu(x_s3_i)
            if self.gm_pool  == 'on':
                b, c, h, w = x_s3_i.shape
                x_s3_i = x_s3_i.view(b, c, -1)
                p = 3.0
                x_s3_i = (torch.mean(x_s3_i**p, dim=-1) + 1e-12)**(1/p)
            else:
                x_s3_i = self.avgpool(x_s3_i)
                x_s3_i = x_s3_i.view(x_s3_i.size(0), x_s3_i.size(1))
            x_s3_i_f = x_s3_i
            ...
            x_s4_r = self.conv_basis4(x1_s4, x2_s4_f_new)
            x1_s4_new = self.Conv1x1_4(x1_s4)
            x_s4_r = x_s4_r+x1_s4_new
            x_s4_r = self.relu(x_s4_r)
            if self.gm_pool  == 'on':
                b, c, h, w = x_s4_r.shape
                x_s4_r = x_s4_r.view(b, c, -1)
                p = 3.0
                x_s4_r = (torch.mean(x_s4_r**p, dim=-1) + 1e-12)**(1/p)
            else:
                x_s4_r = self.avgpool(x_s4_r)
                x_s4_r = x_s4_r.view(x_s4_r.size(0), x_s4_r.size(1))
            x_s4_r_f = x_s4_r
            
            #s4 rgb to ir
            x1_s4_f_new = self.id_fc2(x1_s4_f)
            x1_s4_f_new = self.id_tanh(x1_s4_f_new)
            x_s4_i = self.conv_basis4(x2_s4, x1_s4_f_new)
            x2_s4_new = self.Conv1x1_4(x2_s4)
            x_s4_i = x_s4_i+x2_s4_new
            x_s4_i = self.relu(x_s4_i)
            if self.gm_pool  == 'on':
                b, c, h, w = x_s4_i.shape
                x_s4_i = x_s4_i.view(b, c, -1)
                p = 3.0
                x_s4_i = (torch.mean(x_s4_i**p, dim=-1) + 1e-12)**(1/p)
            else:
                x_s4_i = self.avgpool(x_s4_i)
                x_s4_i = x_s4_i.view(x_s4_i.size(0), x_s4_i.size(1))
            x_s4_i_f = x_s4_i
        
            x_s4_f = torch.cat((x_s4_r_f,x_s4_i_f),0)
        ...
        # x = self.dropout(x)
            ...
            x_s3_f = self.fc_output1(x_s3_f)
            x_s4_f = self.fc_output2(x_s4_f)
        ...
        if self.training:
            return (out_0, out_1, out_2, out_3, out_4, out_5,out_s3,out_s4), (self.l2norm(y_0), self.l2norm(y_1), self.l2norm(y_2), self.l2norm(y_3), self.l2norm(y_4), self.l2norm(y_5),self.l2norm(y_s3),self.l2norm(y_s4))
        else:
            y_0 = self.l2norm(y_0)
            y_1 = self.l2norm(y_1)
            y_2 = self.l2norm(y_2)
            ...
            y = torch.cat((y_0, y_1, y_2, y_3, y_4, y_5,y_s3,y_s4), 1)
            return  y
