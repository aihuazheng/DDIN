...
class HighOrderAttention(nn.Module):
    def __init__(self, in_channels, num_orders = 4):
        super(HighOrderAttention, self).__init__()
        self.num_orders = num_orders
        self.inter_channels = in_channels // 4   
         
        for i in range(self.num_orders):
            name = 'order' + str(self.num_orders) + "_" + str(i+1) 
        ...
        for k in range(self.num_orders):
            name = "convb" + str(self.num_orders) + "_" + str(k+1)
    ...
    def forward(self, x):
        ...
        #1th avg
        for i in range(self.num_orders):
          y_sum = y_sum+inter_feat1[sub]
          sub += 1
        y_avg = y_sum/4
        inter_feat2.append(F.relu(y_avg )) 
        #pdb.set_trace()
        
        #2th avg_dot2
        sum_dot2  = 0
        for i ,j in combinations(inter_feat1,2):
            sum_dot2 = i*j +sum_dot2
        avg_dot2 = sum_dot2 / 6
        inter_feat2.append(F.relu(avg_dot2)) 
        
        #3th avg_dot3
        sum_dot3 = 0
        for i,j,k in combinations(inter_feat1,3):
            sum_dot3  = i * j * k + sum_dot3
        avg_dot3 = sum_dot3 /4
        inter_feat2.append(F.relu(avg_dot3))
       
        #4th dot
        sub = 0
        y_mid = 1
        for j in range(self.num_orders):
            y_mid = y_mid * inter_feat1[sub]
            sub += 1
        inter_feat2.append(F.relu(y_mid ))
        ...
        for k in range(self.num_orders):
            name = "convb" + str(self.num_orders) + "_" + str(k+1)
            layer = getattr(self, name)
            y += layer(inter_feat2[k])
        out = x * y / self.num_orders
        return out  
...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50'):
        ...
        for i in range(4):
            #for j in range(i+1):
                 name = "HAM_" + str(i+1)
                 #setattr(self, name, HighOrderAttention(512, i+1))
                 setattr(self, name, HighOrderAttention(256 * (int(math.pow(2,i))), i+1))
        
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            #pdb.set_trace()
            x1_s3,x1_s3_f,x1_s4 = self.visible_net(x1)
            x2_s3,x2_s3_f,x2_s4 = self.thermal_net(x2)
            
            x2_s3_f_new= self.id_fc1(x2_s3_f)
            x2_s3_f_new = self.id_tanh(x2_s3_f_new)
            x_s3_r = self.conv_basis(x1_s3, x2_s3_f_new)
            ...
            x1_s3_f_new = self.id_fc2(x1_s3_f)
            x1_s3_f_new = self.id_tanh(x1_s3_f_new)
            x_s3_i = self.conv_basis(x2_s3, x1_s3_f_new)
        ...
        # x = self.dropout(x)
            ...
            #pdb.set_trace()
            x_s3,x_s3_f,x_s4 = self.visible_net(x1)
            ...
            x_s3,x_s3_f,x_s4 = self.thermal_net(x2)
            x_s3_f = self.fc_output(x_s3_f)
            '''
            x_s4_f = self.id_fc(x_s4_f)
            x_s4_f = self.id_tanh(x_s4_f)
