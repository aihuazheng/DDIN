...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/sysu_cat_layers_nolr/', type=str, 
...
ignored_params = list(map(id, net.feature1.parameters())) \
                 ...
                 + list(map(id, net.featurecat.parameters())) \
                 ...
                 + list(map(id, net.classifiercat.parameters()))
...
if args.optim == 'sgd':
    optimizer = optim.SGD([
         ...
         {'params': net.featurecat.parameters(), 'lr': args.lr},
         ...
         {'params': net.classifiercat.parameters(), 'lr': args.lr}],
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        #labelscat = torch.cat((label1,label2,label1),0)
        #labelscat = Variable(labelscat.cuda().long())
        ...
        optimizer.zero_grad()
        torch.autograd.backward([loss0, loss1, loss2, loss3, loss4, loss5,loss6], [torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda()])
        #loss.backward()
        optimizer.step()
...
def test(epoch):   
    ...
    #gall_y_cat = np.zeros((ngall, args.low_dim))
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            feat_pool, feat,x_cat = net(input, input, test_mode[0])
            gall_feat[ptr:ptr+batch_num,: ] = feat.detach().cpu().numpy()
            #gall_y_cat[ptr1:ptr1+batch_num,: ] = y_cat.detach().cpu().numpy()
            ptr = ptr + batch_num
            #ptr1 = ptr1 + batch_num
    ...
    #query_y_cat = np.zeros((nquery, args.low_dim))
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            feat_pool, feat,x_cat = net(input, input, test_mode[1])
            query_feat[ptr:ptr+batch_num,: ] = feat.detach().cpu().numpy()
            #query_y_cat[ptr1:ptr1+batch_num,: ] = y_cat.detach().cpu().numpy()
            ptr = ptr + batch_num   
            #ptr1 = ptr1 + batch_num      
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
    ...
    #distmat1  = np.matmul(query_y_cat, np.transpose(gall_y_cat))
    ...
    if dataset =='regdb':
        ...
        #cmc1,mAP1 = eval_sysu(-distmat1, query_label, gall_label, query_cam, gall_cam)
        #cmc = cmc+cmc1
        #mAP = mAP+mAP1
