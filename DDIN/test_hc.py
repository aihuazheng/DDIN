...
from model_hc import embed_net
from utils import *
import time 
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/Baseline/', type=str, help='model save path')
...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            pool_feat, feat,x3 = net(input, input, test_mode[0])
...
def extract_query_feat(query_loader):
    ...
    query_s3 = np.zeros((nquery,2*feature_dim,18,9))
    query_label = np.zeros((nquery))
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            pool_feat, feat,x3 = net(input, input, test_mode[1])
            query_feat[ptr:ptr+batch_num,: ] = feat.detach().cpu().numpy()
            query_feat_pool[ptr:ptr+batch_num,: ] = pool_feat.detach().cpu().numpy()
            query_s3[ptr:ptr+batch_num,:,:,:] = s3.detach().cpu().numpy()
            query_label[ptr:ptr+batch_num] = label.detach().cpu().numpy()
            ptr = ptr + batch_num         
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
    print('Extracting Time:\t {:.3f}'.format(time.time() - start), file=test_log_file)
    return query_feat, query_feat_pool ,query_s3,query_label 
    
query_feat, query_feat_pool,query_s3,query_label = extract_query_feat(query_loader)   
#pdb.set_trace()
query_s3 = torch.from_numpy(query_s3)
query_cam6_0027_0018 = query_3[237:238,:,:,:]
query_label_now = query_label[237]
print(query_label_now)
view_query_s3_hu = torch_vis_color('sysu_baseline_cam6_0027_0018',query_cam6_0027_0018,32,32,'/DATA/fengmengya/save_image/query_baseline/',colormode=2,margining=1)   
