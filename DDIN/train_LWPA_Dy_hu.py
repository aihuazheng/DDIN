...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/LWPA_Dy_HU/', type=str, 
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        if args.method =='id':
            ...
            #outputs[6] is the feature of LWPA module 
            loss_id_att = criterion(outputs[6], labels)
            #outputs[7] is the feature of DY_hu module
            loss_id_s3 = criterion(outputs[7], labels)
        ...
        optimizer.zero_grad()
        torch.autograd.backward([loss0, loss1, loss2, loss3, loss4, loss5,loss_id_att,loss_id_s3], [torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda()])
        #loss.backward()
        optimizer.step()
       
        loss = (loss0 + loss1 + loss2 + loss3 + loss4 + loss5 + loss_id_att +  loss_id_s3 ) / 8
