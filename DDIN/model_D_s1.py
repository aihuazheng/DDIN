...
class visible_net_resnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_s1 = x
        s1_f = F.adaptive_avg_pool2d(x_s1,(1,1))
        s1_f = s1_f.view(s1_f.size(0), s1_f.size(1))
...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50'):
        ...
        self.fc_output1 = nn.Linear(256,512)
        ...
        self.classifiers1 = ClassBlock(low_dim, class_num, dropout=drop)
        #self.classifiers2 = ClassBlock(low_dim, class_num, dropout=drop)
       # self.classifiers3 = ClassBlock(low_dim, class_num, dropout=drop)

        self.l2norm = Normalize(2)
        self.id_fc1 = nn.Linear(256, 64)
        self.id_fc2 = nn.Linear(256, 64)
        #self.id_fc3 = nn.Linear(1024, 64)
        self.id_tanh = nn.Tanh()
        self.conv_basis= ConvBasis2d(64,256, 512, kernel_size=1, padding=0, bias=False)
        #self.conv_basis2= ConvBasis2d(64,512, 512, kernel_size=1, padding=0, bias=False)
        ...
        #self.fc_output3 = nn.Linear(1024,512)
        self.num_part = num_part(6)
        self.avgpool1= nn.AdaptiveAvgPool2d((1,1))
        #self.avgpool2 = nn.AdaptiveAvgPool2d((1,1))
        #self.avgpool3 = nn.AdaptiveAvgPool2d((1,1))
        self.Conv1x1_r = Conv1x1(256,512)
        self.Conv1x1_i = Conv1x1(256,512)
        
        
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            #pdb.set_trace()
            x1_s1,x1_s1_f,x1_s4= self.visible_net(x1)
            x2_s1,x2_s1_f,x2_s4 = self.thermal_net(x2)
            #1*1
            
            
            #s1
            x1_s1_f_new =  self.id_fc1(x1_s1_f)
            x1_s1_f_new =  self.id_tanh(x1_s1_f_new)
            x_s1_r_local = self.conv_basis(x1_s1, x1_s1_f_new)
            
            
            x2_s1_f_new = self.id_fc2(x2_s1_f)
            x2_s1_f_new = self.id_tanh(x2_s1_f_new)
            x_s1_i_local = self.conv_basis(x2_s1, x2_s1_f_new)
            
            #HU
            x1_s1 = self.Conv1x1_r(x1_s1)
            x_s1_r = x_s1_i_local+x1_s1
            x_s1_r = self.relu(x_s1_r)
            x_s1_r = F.adaptive_avg_pool2d(x_s1_r,(1,1))
            x_s1_r = x_s1_r.view(x_s1_r.size(0), -1)
            x_s1_r_f = x_s1_r
            
            x2_s1 = self.Conv1x1_i(x2_s1)
            x_s1_i = x_s1_r_local+x2_s1
            x_s2_i = self.relu(x_s1_i)
            x_s1_i = F.adaptive_avg_pool2d(x_s1_i,(1,1))
            x_s1_i = x_s1_i.view(x_s1_i.size(0), -1)
            x_s1_i_f = x_s1_i
            
            x_s1_f = torch.cat((x_s1_r_f, x_s1_i_f), 0)
        ...
        # x = self.dropout(x)
            ...
            #pdb.set_trace()
            x_s1,x_s1_f,x_s4 = self.visible_net(x1)
            
            x_s1_f =  self.fc_output1(x_s1_f)
            ...
            x_s1,x_s1_f,x_s4 = self.thermal_net(x2)
            x_s1_f = self.fc_output1(x_s1_f)
       ...
       # y_s2 = x_s2_f
        ...
        out_s1 = self.classifiers1(y_s1)
        #out_s1 = self.classifiers2(y_s1)
        #out_s3 = self.classifiers3(y_s3)
        #out = self.classifier(y)
        if self.training:
            return (out_0, out_1, out_2, out_3, out_4, out_5,out_s1), (self.l2norm(y_0), self.l2norm(y_1), self.l2norm(y_2), self.l2norm(y_3), self.l2norm(y_4), self.l2norm(y_5),self.l2norm(y_s1))
        else:
            y_0 = self.l2norm(y_0)
            y_1 = self.l2norm(y_1)
            y_2 = self.l2norm(y_2)
            ...
            y_4 = self.l2norm(y_4)
            y_5 = self.l2norm(y_5)
            y_s1 = self.l2norm(y_s1)
            y = torch.cat((y_0, y_1, y_2, y_3, y_4, y_5,y_s1), 1)
            return  y
