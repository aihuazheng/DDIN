...
class embed_net(nn.Module):
    ...
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            #pdb.set_trace()
            x1_s3,x1_s4 = self.visible_net(x1)
            x2_s3,x2_s4 = self.thermal_net(x2)
            ...
            x_s3_i = self.avgpool_i(x_s3_i)
            x_s3_i = x_s3_i.view(x_s3_i.size(0), -1)
            #x_s3_i_f = x_s3_i
            
            x_s3 = torch.cat((x_s3_r,x_s3_i),0)  #64*1024
        ...
        # x = self.dropout(x)
            ...
            #pdb.set_trace()
            x_s3,x_s4 = self.visible_net(x1)
            x_s3_avg = self.avgpool1(x_s3)
            x_s3 = x_s3_avg.view(x_s3_avg.size(0), x_s3_avg.size(1))      
            ...
            x_s3,x_s4 = self.thermal_net(x2)
            x_s3_avg = self.avgpool2(x_s3)
            x_s3 = x_s3_avg.view(x_s3_avg.size(0), x_s3_avg.size(1))    
            x = self.num_part(x_s4)
            x = x.chunk(6, 2)
            x_0 = x[0].contiguous().view(x[0].size(0), -1)
