...
from model_D_s1 import embed_net
from utils import *
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/Dynamic_s1/', type=str, 
...
parser.add_argument('--gpu', default='0,1,4', type=str,
...
#net = torch.nn.DataParallel(net, device_ids=[2, 3, 4])
#net.to(device)
...
ignored_params = list(map(id, net.feature1.parameters())) \
                 ...
                 + list(map(id, net.fc_output1.parameters())) \
                 ...
                 + list(map(id, net.classifiers1.parameters()))
                 #+ list(map(id, net.classifiers2.parameters()))\
...
if args.optim == 'sgd':
    optimizer = optim.SGD([
         ...
         {'params': net.fc_output1.parameters(), 'lr': args.lr},
         #{'params': net.fc_output3.parameters(), 'lr': args.lr},
         ...
         {'params': net.classifiers1.parameters(), 'lr': args.lr}],
         # {'params': net.classifiers2.parameters(), 'lr': args.lr},
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        #labelsc = torch.cat((label1,label2,label1,label2),0)
        #labelsc = Variable(labelsc.cuda().long())
        data_time.update(time.time() - end)
        
        outputs, feat = model(input1, input2)
        if args.method =='id':
            ...
            #loss_ce = loss0 + loss1 + loss2 + loss3 + loss4 + loss5 + loss6
           ...
           # loss8 = criterion(outputs[8], labels)
            ...
            #loss_center = w_hc * (loss_c0 + loss_c1 + loss_c2 + loss_c3 + loss_c4 + loss_c5)
            
           # loss = loss_ce + loss_center
