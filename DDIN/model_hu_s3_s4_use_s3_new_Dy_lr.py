...
# define  layer4
class layer4_r(nn.Module):
    def __init__(self, arch ='resnet18'):
        super(layer4_r, self).__init__()
        if arch =='resnet18':
...
class layer4_i(nn.Module):
    def __init__(self, arch ='resnet18'):
        super(layer4_i, self).__init__()
        if arch =='resnet18':
            model_ft = models.resnet18(pretrained=True)
        elif arch =='resnet50':
            model_ft = models.resnet50(pretrained=True)
...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50'):
        ...
        if arch =='resnet18':
            ...
            self.visible_net = visible_net_resnet(arch = arch)
            self.thermal_net = thermal_net_resnet(arch = arch)
            self.layer4_r = layer4_r(arch = arch)
            self.layer4_i = layer4_i(arch = arch)
            pool_dim = 2048
            pool_s3 =1024
        ...
        self.conv_basis = DYconv(1024,1024,1,0,reduction=4,K=4,t=30)
        self.relu = nn.ReLU(inplace=True)
       
        self.num_part = num_part(6)
        self.avgpool1= nn.AdaptiveAvgPool2d((1,1))
        self.avgpool2= nn.AdaptiveAvgPool2d((1,1))
        self.avgpool_r = nn.AdaptiveAvgPool2d((1,1))
        self.avgpool_i = nn.AdaptiveAvgPool2d((1,1))
        #self.Conv1x1_r = Conv1x1(1024,512)
        #self.Conv1x1_i = Conv1x1(1024,512)
        #self.id_fc3 = nn.Linear(1024, 512)
        
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            #pdb.set_trace()
            x1_s3 = self.visible_net(x1)
            x2_s3 = self.thermal_net(x2)
       
            x_s3_r = self.conv_basis(x1_s3)
          
            x_s3_r = x_s3_r+x1_s3
            x1_s4 = self.layer4_r(x_s3_r)
        
            x_s3_i = self.conv_basis(x2_s3)
        
            x_s3_i = x_s3_i+x2_s3
            x2_s4 = self.layer4_i(x_s3_i)
        ...
        # x = self.dropout(x)
            ...
            #pdb.set_trace()
            x_s3 = self.visible_net(x1)
            x_s4 = self.layer4_r(x_s3)
            ...
            x_s3 = self.thermal_net(x2)
            x_s4 = self.layer4_i(x_s3)
