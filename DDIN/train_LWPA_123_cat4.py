...
from model_LWPA_123_cat4 import embed_net
from utils import *
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/sysu_LWPA_123_cat4_nolr/', type=str, 
...
ignored_params = list(map(id, net.feature1.parameters())) \
                 ...
                 + list(map(id, net.feature.parameters())) \
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        labelatt = torch.cat((label1,label2,label1,label2,label1,label2,label1,label2),0)
        labelatt = Variable(labelatt.cuda().long())
...
def test(epoch):   
    ...
    #gall_y_layer_att = np.zeros((ngall, args.low_dim))
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            feat_pool, feat,layer_att = net(input, input, test_mode[0])
            gall_feat[ptr:ptr+batch_num,: ] = feat.detach().cpu().numpy()
            ptr = ptr + batch_num
            #gall_y_layer_att[ptr1:ptr1+4*batch_num,: ] = y_layer_att.detach().cpu().numpy()
            #ptr1 = ptr1 + 4*batch_num
    ...
    #query_y_layer_att = np.zeros((nquery, args.low_dim))
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            feat_pool, feat,layer_att = net(input, input, test_mode[1])
            query_feat[ptr:ptr+batch_num,: ] = feat.detach().cpu().numpy()
            ptr = ptr + batch_num 
            #query_y_layer_att[ptr1:ptr1+4*batch_num,: ] = y_layer_att.detach().cpu().numpy()
            #ptr1 = ptr1 + 4*batch_num         
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
    ...
    #distmat1  = np.matmul(query_y_layer_att, np.transpose(gall_y_layer_att))
