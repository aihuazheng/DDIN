...
from model_xcross import embed_net
from utils import *
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/HEloss_KLloss', type=str, 
...
ignored_params = list(map(id, net.feature1.parameters())) \
                 ...
                 + list(map(id, net.features34.parameters())) \
                 ...
                 + list(map(id, net.classifiers34.parameters()))
...
if args.optim == 'sgd':
    optimizer = optim.SGD([
         ...
         {'params': net.features34.parameters(), 'lr': args.lr},
         ...
         {'params': net.classifiers34.parameters(), 'lr': args.lr}],
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        if args.method =='id':
            ...
            klloss = (kl_loss(outputs[6],outputs[7])+kl_loss(outputs[7],outputs[8]))/ input1.size(0)
        ...
        optimizer.zero_grad()
        torch.autograd.backward([loss0, loss1, loss2, loss3, loss4, loss5,klloss], [torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda()])
        #loss.backward()
        optimizer.step()
        loss = (loss0 + loss1 + loss2 + loss3 + loss4 + loss5+klloss) / 7
