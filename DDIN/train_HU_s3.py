...
from model_s3_hu_lr import embed_net
from utils import *
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/regdb_HU_s3/', type=str, 
...
ignored_params = list(map(id, net.feature1.parameters())) \
                 ...
                 + list(map(id, net.fc_output.parameters())) \
...
if args.optim == 'sgd':
    optimizer = optim.SGD([
         ...
         {'params': net.fc_output.parameters(), 'lr': args.lr},
...
def test(epoch):   
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            feat,s3_hu = net(input, input, test_mode[0])
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            feat,s3_hu = net(input, input, test_mode[1])
