...
class thermal_net_resnet(nn.Module):
    ...
    def forward(self, x):
        ...
        return s1,s2,s3
# Define layer4     
class visible_layer4(nn.Module):
    def __init__(self, arch ='resnet18'):
        super(visible_layer4, self).__init__()
        if arch =='resnet18':
            model_ft = models.resnet18(pretrained=True)
        elif arch =='resnet50':
            model_ft = models.resnet50(pretrained=True)
...
class thermal_layer4(nn.Module):
    def __init__(self, arch ='resnet18'):
        super(thermal_layer4, self).__init__()
        if arch =='resnet18':
            model_ft = models.resnet18(pretrained=True)
        elif arch =='resnet50':
            model_ft = models.resnet50(pretrained=True)
...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50',lma = True):
        ...
        if arch =='resnet18':
            ...
            self.visible_net = visible_net_resnet(arch = arch)
            self.thermal_net = thermal_net_resnet(arch = arch)
            self.visible_l4 = thermal_layer4(arch = arch)
            self.thermal_l4 = thermal_layer4(arch = arch)
            pool_dim = 2048
    ...
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            #pdb.set_trace()
            x1_s1,x1_s2,x1_s3= self.visible_net(x1) 
            x2_s1,x2_s2,x2_s3 =  self.thermal_net(x2)
            
            #hus3
            x1,x1_s4 = self.visible_l4(x2_s3)
            x2,x2_s4 = self.thermal_l4(x1_s3)
            ...
            att_s3 = layer_att
            att_s3 = att_s3.view(att_s3.size(0), att_s3.size(1),1,1)
            ...
            #pdb.set_trace()
            x_s1,x_s2,x_s3= self.visible_net(x1)
            x,x_s4 = self.visible_l4(x_s3)
            ...
            x_s1,x_s2,x_s3= self.thermal_net(x2)
            x,x_s4 = self.thermal_l4(x_s3)
        ...
        if self.training:
            ...
            layer_att =  self.l2norm(layer_att)#32 1024
