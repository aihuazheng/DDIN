...
class visible_net_resnet(nn.Module):
    ...
    def forward(self, x):
       ...
       # x_s1_avg = self.visible.avgpool(x_s1)
        ...
        x_s3 = x
        #x_s3_avg = self.visible.avgpool(x_s3)
        #x_s3_f = x_s3_avg.view(x_s3_avg.size(0), x_s3_avg.size(1))
        ...
        # x = self.dropout(x)
        '''
        return x_s1,x_s2,x_s2_f,x_s3,x_s4
...
class thermal_net_resnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_s3 = x
        #x_s3_avg= self.thermal.avgpool(x_s3)
        #x_s3_f = x_s3_avg.view(x_s3_avg.size(0), x_s3_avg.size(1))
...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50'):
        ...
        self.id_fc1 = nn.Linear(512, 64)
        self.id_fc2 = nn.Linear(512, 64)
        self.id_tanh = nn.Tanh()
        self.conv_basis= ConvBasis2d(64,512, 512, kernel_size=3, padding=0, bias=False)
        ...
        self.Conv3x3_r = Conv3x3(512,512)
        self.Conv3x3_i = Conv3x3(512,512)
        
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            #pdb.set_trace()
            x1_s1,x1_s2,x1_s2_f,x1_s3,x1_s4 = self.visible_net(x1)
            x2_s1,x2_s2,x2_s2_f,x2_s3,x2_s4 = self.thermal_net(x2)
            
            x2_s2_f_new= self.id_fc1(x2_s2_f)
            x2_s2_f_new = self.id_tanh(x2_s2_f_new)
            x_s2_r = self.conv_basis(x1_s2, x2_s2_f_new)
            x1_s2 = self.Conv3x3_r(x1_s2)
            x_s2_r = x_s2_r+x1_s2
            x_s2_r = self.relu(x_s2_r)
            x_s2_r = self.avgpool(x_s2_r)
            x_s2_r = x_s2_r.view(x_s2_r.size(0), -1)
            x_s2_r_f = x_s2_r
            
            x1_s2_f_new = self.id_fc2(x1_s2_f)
            x1_s2_f_new = self.id_tanh(x1_s2_f_new)
            x_s2_i = self.conv_basis(x2_s2, x1_s2_f_new)
            x2_s2 = self.Conv3x3_i(x2_s2)
            x_s2_i = x_s2_i+x2_s2
            x_s2_i = self.relu(x_s2_i)
            x_s2_i = self.avgpool(x_s2_i)
            x_s2_i = x_s2_i.view(x_s2_i.size(0), -1)
            x_s2_i_f = x_s2_i
            
            
            #x2_s3_f = self.fc_output(x2_s3_f)
            x_s2_f = torch.cat((x_s2_r_f,x_s2_i_f),0)
            '''
        ...
        # x = self.dropout(x)
            ...
            #pdb.set_trace()
            x_s1,x_s2,x_s2_f,x_s3,x_s4 = self.visible_net(x1)
            x_s2_f = x_s2_f
            #x_s2_f = self.fc_output(x_s3_f)
            '''
            x_s4_f = self.id_fc(x_s4_f)
            x_s4_f = self.id_tanh(x_s4_f)
            x_s1_new = self.conv_basis(x_s1, x_s4_f)
            x_s1_new = self.relu(x_s1_new)
            x_s1_new = self.avgpool(x_s1_new)
            x_s1_new = x_s1_new.view(x_s1_new.size(0), -1)
            x_s1_f = self.fc_output(x_s1_new)
            x_f = torch.cat((x_s1_f, x_s4_f), 0)
            '''
            ...
            x_s1,x_s2,x_s2_f,x_s3,x_s4 = self.thermal_net(x2)
            #x_s3_f = self.fc_output(x_s3_f)
            x_s2_f = x_s2_f
            '''
            x_s4_f = self.id_fc(x_s4_f)
            x_s4_f = self.id_tanh(x_s4_f)
        ...
        out_s2 = self.classifier(y_s2)
        #out = self.classifier(y)
        if self.training:
            return (out_0, out_1, out_2, out_3, out_4, out_5,out_s2), (self.l2norm(y_0), self.l2norm(y_1), self.l2norm(y_2), self.l2norm(y_3), self.l2norm(y_4), self.l2norm(y_5),self.l2norm(y_s2))
        else:
            y_0 = self.l2norm(y_0)
            y_1 = self.l2norm(y_1)
            y_2 = self.l2norm(y_2)
            ...
            y = torch.cat((y_0, y_1, y_2, y_3, y_4, y_5,y_s2), 1)
            return  y
