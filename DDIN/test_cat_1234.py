...
from  model_cat_1234 import embed_net
from utils import *
import time 
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/sysu_cat_1234_nolr/', type=str, help='model save path')
...
start_epoch = 0 
print('==> sysu_cat_1234_nolr_all_multi..')
...
def extract_gall_feat(gall_loader):
    ...
    gall_x_cat = np.zeros((ngall, 4*pool_dim))
    with torch.no_grad():
...
def extract_query_feat(query_loader):
    ...
    query_x_cat = np.zeros((nquery, 4*pool_dim))
    with torch.no_grad():
