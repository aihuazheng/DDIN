...
from model_dy_nohu_nouploss import embed_net
from utils import *
...
parser.add_argument('--model_path', default='/home/panpeng/fmy/save_model/base_ye/dy_nohu_nouploss/', type=str, 
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        if args.method =='id':
            ...
            #pdb.set_trace()
            #loss_tri_s3 = criterion_tri(feat[6],labels)
        ...
        optimizer.zero_grad()
        torch.autograd.backward([loss0, loss1, loss2, loss3, loss4, loss5,loss_id_s3], [ torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda(),torch.tensor(1.0).cuda()])
        #loss.backward()
        optimizer.step()
       
        loss = (loss0 + loss1 + loss2 + loss3 + loss4 + loss5 ) / 6 + loss_id_s3
