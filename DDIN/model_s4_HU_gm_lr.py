...
class embed_net(nn.Module):
    def __init__(self, low_dim, class_num, drop = 0.5, arch ='resnet50',gm_pool = 'on'):
        ...
        self.conv_basis= ConvBasis2d(64,2048, 512, kernel_size=1, padding=0, bias=False)
        #self.conv_basis_i= ConvBasis2d(1024,1024, 512, kernel_size=1, padding=0, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.fc_output = nn.Linear(2048,512)
        self.num_part = num_part(6)
        self.avgpool= nn.AdaptiveAvgPool2d((1,1))
        self.avgpool2 = nn.AdaptiveAvgPool2d((1,1))
        self.gm_pool = gm_pool
        self.Conv1x1_r = Conv1x1(2048,512)
        self.Conv1x1_i = Conv1x1(2048,512)
        
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            #pdb.set_trace()
            x1_s4 ,x1_s4_f= self.visible_net(x1)
            x2_s4 ,x2_s4_f= self.thermal_net(x2)
            
            x2_s4_f_new= self.id_fc1(x2_s4_f)
            x2_s4_f_new = self.id_tanh(x2_s4_f_new)
            x_s4_r = self.conv_basis(x1_s4, x2_s4_f_new)
            x1_s4_new = self.Conv1x1_r(x1_s4)
            ...
            x_s4_i = self.conv_basis(x2_s4, x1_s4_f_new)
            x2_s4_new = self.Conv1x1_i(x2_s4)
        ...
        # x = self.dropout(x)
            ...
            #pdb.set_trace()
            x_s4,x_s4_f = self.visible_net(x1)
            x_s4_f = self.fc_output(x_s4_f)
            '''
            ...
            x_s4,x_s4_f = self.thermal_net(x2)
            x_s4_f = self.fc_output(x_s4_f)
            '''
            x_s4_f = self.id_fc(x_s4_f)
            x_s4_f = self.id_tanh(x_s4_f)
        ...
        out_s4 = self.classifier(y_s4)
        #out = self.classifier(y)
        if self.training:
            return (out_0, out_1, out_2, out_3, out_4, out_5,out_s4), (self.l2norm(y_0), self.l2norm(y_1), self.l2norm(y_2), self.l2norm(y_3), self.l2norm(y_4), self.l2norm(y_5),self.l2norm(y_s4))
        else:
            y_0 = self.l2norm(y_0)
            y_1 = self.l2norm(y_1)
            y_2 = self.l2norm(y_2)
            ...
            y = torch.cat((y_0, y_1, y_2, y_3, y_4, y_5,y_s4), 1)
            return  y
