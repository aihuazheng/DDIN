...
from model_multi_classifer import embed_net
from utils import *
...
parser.add_argument('--model_path', default='/DATA/fengmengya/save_model/sysu_multi_classifer_nolr/', type=str, 
...
print('==> sysu_multi_classifer_nolr..')
print('==> Loading data..')
# Data loading code
...
ignored_params = list(map(id, net.feature1.parameters())) \
                 ...
                 + list(map(id, net.features1.parameters())) \
                 + list(map(id, net.features2.parameters())) \
...
if args.optim == 'sgd':
    optimizer = optim.SGD([
         ...
         {'params': net.features1.parameters(), 'lr': args.lr},
         {'params': net.features2.parameters(), 'lr': args.lr},
...
def train(epoch, loss_log):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        torch.autograd.backward([loss0, loss1, loss2, loss3, loss4, loss5,loss6,loss7,loss8], [torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda()])
        #loss.backward()
        optimizer.step()
        loss = (loss0 + loss1 + loss2 + loss3 + loss4 + loss5+ loss6+ loss7+ loss8) / 9
...
def test(epoch):   
    ...
    start = time.time()
    ptr = 0
    gall_feat = np.zeros((ngall, 9*args.low_dim))
    with torch.no_grad():
    ...
    start = time.time()
    ptr = 0
    query_feat = np.zeros((nquery, 9*args.low_dim))
    with torch.no_grad():
