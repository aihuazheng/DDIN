...
class visible_net_resnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_s1 = x
        #x_s1 = self.visible.avgpool(x_s1)
        ...
        # x = self.dropout(x)
        '''
        return x_s1,x_s2,x_s3,x_s3_f,x_s4, x_s4_f
...
class thermal_net_resnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_s1 = x
        #x_s1 = self.thermal.avgpool(x_s1)
        #x_s1 = x_s1.view(x_s1.size(0), x_s1.size(1))
        x = self.thermal.layer2(x)
        x_s2 = x
        #x_s2_avg = self.thermal.avgpool(x_s2)
        #x_s2_f = x_s2.view(x_s2_avg.size(0), x_s2_avg.size(1))
...
class embed_net(nn.Module):
    ...
    def forward(self, x1, x2, modal = 0 ):
        if modal==0:
            #pdb.set_trace()
            x1_s1,x1_s2,x1_s3,x1_s3_f,x1_s4,x1_s4_f = self.visible_net(x1)
            x2_s1,x2_s2,x2_s3,x2_s3_f,x2_s4,x2_s4_f = self.thermal_net(x2)
            x1_s3_f_new= self.id_fc(x1_s3_f)
            ...
            x_s3_i_f = x_s3_i
            '''
            x_s3_i = self.conv_basis(x2_s3, x1_s3)
            x2_s3 = self.Conv1x1_i(x2_s3)
            x_s3_i = x_s3_i+x2_s3
            x_s3_i = self.relu(x_s3_i)
            x_s3_i = self.avgpool(x_s3_i)
            ...
            x1_s3_f = self.fc_output(x1_s3_f)
            x_s3_f = torch.cat((x_s3_i_f,x1_s3_f),0)
            '''
        ...
        # x = self.dropout(x)
            ...
            #pdb.set_trace()
            x_s1,x_s2,x_s3,x_s3_f,x_s4,x_s4_f = self.visible_net(x1)
            ...
            x_s1,x_s2,x_s3,x_s3_f,x_s4,x_s4_f = self.thermal_net(x2)
            x_s3_f = self.fc_output(x_s3_f)
            '''
            x_s4_f = self.id_fc(x_s4_f)
            x_s4_f = self.id_tanh(x_s4_f)
